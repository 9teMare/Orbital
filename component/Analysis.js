...
import { View, useWindowDimensions, Image, Text, ActivityIndicator, Dimensions } from 'react-native';
...
export default function Analysis({ route }) {
  ...
  const fetching = async () => {
    ...
    for (var i=0; i<5; i++) {
      ...
      //from ddragon
      const response_dd_blue = await fetch("http://ddragon.leagueoflegends.com/cdn/11.15.1/data/en_US/champion/" + blueName + ".json")
      const responded_dd_blue = await response_dd_blue.json()
      const data_dd_blue = responded_dd_blue.data

      const response_dd_red = await fetch("http://ddragon.leagueoflegends.com/cdn/11.15.1/data/en_US/champion/" + redName + ".json")
  ...
  return (
    ...
    <Tab.Navigator
      ...
      labeled={false}>
      <Tab.Screen name='Lane' component={laneScreen} initialParams={{ blue: blue, red: red, ddragon: ddragon, cdn: cdn, otherInfo: OtherInfo}}
        options={{
          tabBarIcon: ({ color }) => (
