...
import { View, Text, Button, FlatList, Image, ScrollView, StyleSheet } from 'react-native'
...
export default function adviceBlue({ route }) {
    ...
    const fetching = async () => {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const tagArr = ddragon["blue"][i]["tags"]
            for (let j = 0; j < tagArr.length; j++) {
                roles.push(tagArr[j])
            }

            //getting ally Tips
            const allyTips_individual = ddragon["blue"][i]["allytips"]
            allyTips.push(allyTips_individual)

            //getting enemy tips
            const enemyTips_individual = ddragon["red"][i]["enemytips"]
